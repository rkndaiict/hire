...
@Service
public class NewHireEnrollmentReminderEmailEvent extends AbstractEventHandler implements EventStrategy {

    @Override
    public boolean appendTokenIdentifierToURLUpdateNotification(NotificationDTO notificationDTO) {

        if (CollectionUtils.isNotEmpty(notificationDTO.getParamList())) {
            for (MapDTO mapDTO : notificationDTO.getParamList()) {
                if(mapDTO.getMap().get(REGISTRATION_URL) == null){
                    continue;
                }
                ...
                saveLinkMetaData(new LinkMetaData(), tokenIdentifier, true, SINGLE, NOTIFICTAION_EVENT_TYPE.EMPLOYEE_ENROLLMENT_REMINDER.getValue(),
                    externalIdentifier);
            }
        }
        return true;
