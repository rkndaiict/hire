...
@Service
public class EmployerPublishEvent extends AbstractEventHandler implements
		EventStrategy {
	...
	public boolean appendTokenIdentifierToURLUpdateNotification(
		...
		if (CollectionUtils.isNotEmpty(notificationDTO.getParamList())) {
			for (MapDTO mapDTO : notificationDTO.getParamList()) {
				...
				saveLinkMetaData(new LinkMetaData(), tokenIdentifier, true, SINGLE, NOTIFICTAION_EVENT_TYPE.EP
